...
#include <memory>
#include <limits>
#include "nav2_bt_navigator/navigators/follow_point.hpp"

namespace nav2_bt_navigator
{

bool
FollowPointNavigator::configure(
  ...
  std::shared_ptr<nav2_util::OdomSmoother> odom_smoother)
{
  ...
  goal_sub_ = node->create_subscription<geometry_msgs::msg::PoseStamped>(
    ...
    std::bind(&FollowPointNavigator::onFollowPointReceived, this, std::placeholders::_1));
  return true;
}

std::string
FollowPointNavigator::getDefaultBTFilepath(
  rclcpp_lifecycle::LifecycleNode::WeakPtr parent_node)
{
  std::string default_bt_xml_filename;
  auto node = parent_node.lock();

  if (!node->has_parameter("default_follow_point_bt_xml")) {
    std::string pkg_share_dir =
      ament_index_cpp::get_package_share_directory("nav2_bt_navigator");
    node->declare_parameter<std::string>(
      "default_follow_point_bt_xml",
      pkg_share_dir +
      "/behavior_trees/follow_point.xml");
  }

  node->get_parameter("default_follow_point_bt_xml", default_bt_xml_filename);

  return default_bt_xml_filename;
}

bool
FollowPointNavigator::cleanup()
{
  goal_sub_.reset();
  self_client_.reset();
  return true;
...
bool
FollowPointNavigator::goalReceived(ActionT::Goal::ConstSharedPtr goal)
{
...
void
FollowPointNavigator::goalCompleted(
...
void
FollowPointNavigator::onPreempt(ActionT::Goal::ConstSharedPtr goal)
{
...
void
FollowPointNavigator::initializeGoalPose(ActionT::Goal::ConstSharedPtr goal)
{
  geometry_msgs::msg::PoseStamped current_pose;
  nav2_util::getCurrentPose(
    current_pose, *feedback_utils_.tf,
...
void
FollowPointNavigator::onFollowPointReceived(const geometry_msgs::msg::PoseStamped::SharedPtr pose)
{
  ActionT::Goal goal;
  goal.pose = *pose;
